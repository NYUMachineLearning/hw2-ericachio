...
train <- sample(nrow(airquality), 0.75*(nrow(airquality)))
...
#get coefficients, do abs line w coefficients
...
```{r}
ggplot(data = train_regression) +
  geom_point(aes(x=Wind, y=Temp)) + geom_abline(intercept = linear_regression$finalModel$coefficients[1], slope = linear_regression$finalModel$coefficients[2], col = 'red') +
  theme_bw() 
...
plot(test_regression$Wind, linear_predict) + abline( 89.556989, -1.168743)


# plot the original test values
plot(test_regression$Wind, test_regression$Temp) + abline( 89.556989, -1.168743)
#anna code
# linear_predict <- predict(linear_regression, newdata=test_regression)
...
ggplot(data=test_regression)+
  geom_abline(aes(intercept =linear_regression$finalModel$coefficients[1], slope = linear_regression$finalModel$coefficients[2], col="Regression Line")) +
...
ggplot(data=test_regression)+ 
  geom_point(aes(x = Wind, y = Temp), col = " red") + geom_point(aes(x = Wind, y = lasso_predict), col="blue")

ggplot(data=test_regression)+ 
  geom_point(aes(x = Month, y = Temp), col = " red") + geom_point(aes(x = Month, y = lasso_predict), col="blue")
```
